...
public class colecaoDao {
	...
	public void incluir(Colecao colecao) {
		...
		String sqlInsert = "insert into categoria (idUsuario, nome, descricao, imagem) values(?, ?, ?, ?)";
	...
	public void atualizarColecao(Colecao colecao) {
		...
		try(Connection conn = ConnectionFactory.conectar(); CallableStatement stm = (CallableStatement) conn.prepareCall("{call usp_atualizarColecao(?, ?, ?, ?)}")){
	...
	public Colecao buscar(Colecao colecao) {
		...
		try (Connection conn = ConnectionFactory.conectar();
			...
			try (ResultSet rs = stm.executeQuery();){
				if (rs.next()) {
					colecao.setIdUsuario(rs.getInt(1));
					colecao.setNome(rs.getString(2));
					colecao.setDescricao(rs.getString(3));
					colecao.setImagem(rs.getBinaryStream(4));
				}
			}
			catch (Exception e) {
				e.printStackTrace();
	...
	public ArrayList<Colecao> buscarColecao() {
		String sqlSelect = "SELECT * from colecao";
	...
	public ArrayList<Colecao> buscarColecao(String chave) {
        String sqlSelect = "select * from colecao where upper(nome) like ?";
